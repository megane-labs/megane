...
| Structure editing — megane Builder (`/builder.html`: open a file by dialog or drop, or start a new document — empty cell or bulk crystal — from the top bar; add / delete / move atoms, bonds, elements with the tool's own settings and keyboard shortcuts; molecule library with presets, Ketcher sketches, file imports and saved selections, placed with *Add* / the *Place* tool; save XYZ / PDB / MOL) | ✓ | — | — | — | — | — |
| Builder *Python tools*: buttons backed by an MCP tool server (liquid box, polymer chain, solvate, … — see [Builder Tool Contract](dev/builder-tools.md)); Streamable HTTP servers only, stdio servers need a host bridge that is not built yet | ✓ | — | — | — | — | — |
